...
| --- | --- | --- |
| **모드 분리(Plan/Build) × 서브에이전트(Explore)** | Plan의 read-only 성격과 Explore의 read-only 위임이 정렬 → 탐색이 메인 컨텍스트를 더럽히지 않음 | Plan/Build가 `agent` alias + `agents: [Explore]` allowlist로 호출 |
| **검증 루프 × 체크포인트/Undo** | 자기수정이 폭주해도 마지막 green 지점으로 롤백 가능 → 자율 편집의 안전망 | 진단+테스트 실패 시 Git 커밋 경계로 복귀 |
| **검증 루프 × eval 북극성(테스트)** | 같은 축의 다른 입자도. "테스트 통과 = 완료" 기준이 자기수정 루프에 명확한 종료조건 제공 | 패치+컨테이너 테스트를 완료 정의로(02 §3.6) |
| **단일 always-on 메모리 × 규칙/스킬** | 프로젝트 관례가 모든 모드에 일관 주입 → 모드가 바뀌어도 행동 일관 | `copilot-instructions.md`(택1) + 경로별 `*.instructions.md` |
...
- **해소**: **load-bearing 컨텍스트 보호**. 현재 diff·최근 테스트 출력·활성 파일·계획(plan)은 요약 대상에서 제외하고, **과거 탐색 로그만** 압축. 계획은 **스크래치(scratch)에 외부화**한다 — 예: 세션 메모리, PR 설명, `.copilot/plan.md` 같은 작업용 파일. **항상 로딩되는 `copilot-instructions.md`/`AGENTS.md`에는 넣지 않는다**(그곳은 변하지 않는 규칙 전용 — 휠발성 계획을 넣으면 매 턴 컨텍스트 낭비·규칙 오염).
...
- **업데이트(2026-06)**: 기계적 강제의 *일부*는 CI를 기다릴 필요 없이 **로컬 Agent hooks**(VS Code, §C13)로 당겨올 수 있다 — `PreToolUse` 차단·`Stop` 검증 게이트는 대화형 세션에서도 동작한다. CI(claude-code-action)는 **사람 없는** 환경에서 같은 가드를 한 번 더 적용하는 상위 레이어로 남는다(예: `harness-doctor`를 PR 게이트로).
...
### C13. 기계적 강제(Hooks) ↔ 미니멀리즘·자제 + 거버넌스 드리프트 — 축 D2 **(전제 변경)**
- **전제 변경**: 이전 설계는 "GHCP엔 hooks가 없다"를 전제로 기계적 강제를 CI 로드맵으로 미뤘다. VS Code Copilot이 **Agent hooks(Preview, 8 이벤트)**를 정식 지원하므로(02 §3.9), 선언적 규칙(도구 allowlist·프롬프트 정책)을 **로컬에서 기계가 강제**할 수 있게 됐다. 즉 P9(기계적 강제)가 실현 가능해졌다.
- **충돌**: hook은 강력하지만 **무한히 늘릴 수 있다.** "안전을 위해" hook을 더하면 mini-swe-agent식 미니멀리즘(C5)이 깨지고, 결정 표면이 아니라 **정책 표면이 비대**해져 유지보수·디버깅 비용이 폭증한다. 또 hook은 *모델 바깥*의 코드라 그 자신이 드리프트한다(문서는 "막는다"는데 코드는 안 막음).
- **의도와 다른 결과**:
  1. **과발화 → 경보 피로**: 정규식이 부정확해 무관한 맥락(예: "핸드오프 작성해줘")에 끼어들면, 모델이 가드를 무시하도록 학습한다.
  2. **감지 즉시 자동 실행**: 부수효과 큰 작업(재빌드·배포·push)을 hook이 자동 실행하면 thrash·무단 재기동으로 C1을 위반한다.
  3. **거버넌스 드리프트**: "protect-paths가 X를 보호한다"는 문서 주장과 실제 차단 경로가 갈라지면, **가장 신뢰가 필요한 자산이 무방비인데 보호된다고 착각**한다(가장 위험).
- **근거**: wikidocs 12.5 원칙 6("하네스 자신도 드리프트한다"), 12.4 ④("관찰된 실패에서 하네스가 나온다 — 미숙한 하네스는 추측"), Anthropic "단순함 유지, 결과가 좋아질 때만 복잡성 추가"(C5와 동일 축).
- **해소(자제 원칙)**:
  1. **관찰된 실패에서만 hook을 추가**한다. "가능하니까"가 아니라 "이 실패가 반복됐으니까". 새 hook은 막으려는 구체적 실패 모드를 명시한다.
  2. **감지 ≠ 자동 실행**: 비가역·부수효과 큰 작업은 hook이 **알림만**, 실행은 사람 확인(C1 재확인).
  3. **무한 루프 안전장치**: Stop hook은 `stop_hook_active`로 재진입 통과, 차단 조건은 "미커밋 변경 존재"처럼 **올바른 행동을 하면 저절로 풀리는** 형태로.
  4. **거버넌스 센서**: `harness-doctor`로 "문서 주장 ↔ hook 실제 차단"과 "문서 수치 ↔ 실제"의 정합을 자동 검사(02 §3.11). 코드 드리프트를 센서로 막듯 하네스 드리프트도 센서로 막는다.
  5. **self-eval 회피**: Stop hook의 점검 질문도 같은 LLM이 답하므로, "실제 명령 출력을 인용해서만 통과"를 명시(wikidocs ⚠ 경고).
  6. **ablation**: 새 모델이 나오면 hook을 하나씩 제거하며 같은 작업을 돌려, 품질이 같으면 제거한다(wikidocs 원칙 1·2 — 컴포넌트는 검증을 통과해야 살아남는다).
...
| D4 정밀↔압축 | 압축 허용 | 정밀(탐색 보존) | 정밀(diff·테스트 보호) | 정밀 + 계획 스크래치 외부화 |
...
- **기계적 강제(hooks)는 관찰된 실패에서만 추가**하고, 감지≠자동실행, self-eval 회피, 무한 루프 안전장치를 지킨다. 문서 주장↔hook 실제 차단의 정합을 `harness-doctor`로 검사(C13).
...
| 가능하다고 hook을 전부 추가 | 정책 표면 비대·경보 피로 (C13) | 관찰된 실패에서만, ablation으로 덜어냄 |
| 문서는 "막는다"는데 hook은 안 막음 | 무방비를 보호로 착각, 신뢰 붕괴 (C13) | harness-doctor로 문서↔구현 정합 검사 |
| 검증을 LLM이 자기 자신에게 시킴 | 후한 자가확인으로 미검증 통과 (C13) | 실제 명령 출력을 인용해서만 완료 |
...
2. 불변 규칙을 `.github/copilot-instructions.md`에 명문화(C2·C3·C4·C8·C9·C13).
3. C13의 기계적 강제를 [.github/hooks/](../.github/hooks/)에 구현하고, [scripts/harness-doctor.mjs](../scripts/harness-doctor.mjs)로 문서↔구현 정합을 검사한다.
4. 대표 시나리오로 드라이런하며 상충 매트릭스(§4)의 `–`/`––` 칸이 실제로 해소되는지 검증.
